...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    # IC integration: impersonate stock AP1 so the cluster shows blue lanes + autopilot visuals (HW1 only)
    if self.CP.carFingerprint in (CAR.TESLA_MODEL_S_HW1, CAR.TESLA_MODEL_X_HW1):
      # Phase 1: DAS_lanes at 10Hz — the actual lane geometry
      if self.frame % 10 == 0:
        lanes_cntr = (self.frame // 10) % 16
        can_sends.append(self.tesla_can.create_lane_message(lanes_cntr, CC.enabled))

      # Phase 2: AutopilotStatus + DAS_status2 at 2Hz — tells the IC AP is active so it renders the lanes
      if self.frame % 50 == 0 and CC.enabled:
        status_cntr = (self.frame // 50) % 16
        can_sends.append(self.tesla_can.create_das_status(status_cntr, CC.enabled))
        can_sends.append(self.tesla_can.create_das_status2(status_cntr, CC.enabled))
